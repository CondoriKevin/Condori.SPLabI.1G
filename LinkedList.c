...
LinkedList* ll_filter(LinkedList *this, int (*pFunc)(void *element)) {
	void *pElement;
	LinkedList *aux;//puntero a lincked
	aux = ll_newLinkedList();//crea una nueva lista
	int i;
	for (i = 0; i < ll_len(this); i++) {//recorremos nuestra lista...la que ya teniamos
		pElement = ll_get(this, i);//"tomamos" los elementos que deseamos para filtrar
		if (pFunc(pElement) == 1) {

			ll_add(aux, pElement);//cuando ya tenemos los elementos que queremos... lo pasamos a una nueva lista Filtrada como el cafe
		}
